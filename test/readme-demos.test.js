...
function extractBlocks(source) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!open) {
      ...
      if (match) open = { language: match[1], startLine: index + 1, lines: [] };
      continue;
    }
    if (line === '```') {
      blocks.push({
...
async function runInSmokeSandbox(source, startLine) {
...
async function main() {
  ...
  assert.equal(javascriptBlocks.length, 64, 'README JavaScript demo count changed; review coverage');
